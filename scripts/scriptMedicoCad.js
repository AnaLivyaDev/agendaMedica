...
let labelHora = document.querySelector('#labelHora')
...
//função para validar o valor da consulta
hora.addEventListener('keyup', function () {
    let regexHora = /^([01]\d|2[0-3]):([0-5]\d)(,\s*([01]\d|2[0-3]):([0-5]\d))*$/;

    //Obtém o valor atual do campo
    let valorHora = hora.value;

    if (regexHora.test(valorHora)) {
        labelHora.style.color = 'green';
        hora.style.borderColor = 'green';
        labelHora.innerHTML = 'Digite seus horários de atendimento'
    } else {
